...
运行时只读取 `public/frames/frame-00.png` 到 `frame-30.png`，图片编号由 `score + 15` 得到。视频脚本仅用于重新制作等级图片的源素材，不是网页运行时依赖。
...
`public/frames` 内的人像素材用于本项目的趣味化演示。复用或二次发布前，请确认你拥有相关肖像与素材的使用权。
